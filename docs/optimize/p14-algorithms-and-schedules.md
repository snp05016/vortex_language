...
Every schedule prints `yes` under "same out": the four compute the same image. They differ in three quantities that the paper names as the tension of the whole space: **parallelism**, the work that can run at once; locality, which here means how soon a value is read after it is produced, the producer-and-consumer case of the cache locality in [P7](p7-loop-transformations.md#the-same-work-in-a-different-order); and **redundant recomputation**, work done more than once.[^halide13] Each extreme gives up one of the three. Root loses locality, inline recomputes, sliding serializes. Figure 3 shows where the tiled schedule's extra work comes from.
