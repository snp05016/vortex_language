...
Each dimension must be an integer constant expression (see
...
The compiler checks this form first: a dimension that is not an integer
...
The compiler evaluates each dimension when it resolves the array type, before
...
Every extent must be at least 1. A dimension whose value is zero, whether
...
All elements must have the same type after
...
When an element-list array has an expected type `[T; d1, d2, ..., dn]` with n
...
Vortex array indexing is zero-based. Each index must have an integer type:
...
The number of indices in an index suffix must equal the rank of the indexed
...
When an index is an integer constant expression
...
An index expression evaluates its base and then its indices from left to
...
Arrays are values. Initializing a variable, assigning, passing an argument to a
...
An element may be assigned when its place is mutable
...
In an element assignment, the target's indices are evaluated before the
...
An array value is stored contiguously: its elements occupy consecutive
...
The size and alignment of each element type are implementation-defined and
must be documented
([Conformance](conformance.md#110-implementation-defined-behavior-and-limits)).
A v0.1 program cannot observe addresses, so layout does not change results; it
